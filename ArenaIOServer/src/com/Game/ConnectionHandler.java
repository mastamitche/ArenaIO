...
public class ConnectionHandler {
    ...
    protected void onClose(WebSocket conn, int code, String reason, boolean remote){
        System.out.println("Closed! " + conn);
        if (((WebSocketImpl)conn).connectionHandler.player != null)
        	((WebSocketImpl)conn).connectionHandler.server.removePlayer(((WebSocketImpl)conn));
    ...
    public void onMessage(WebSocket conn, ByteBuffer blob)
    {
    	try {
	    	if (!conn.isOpen() || conn.isConnecting()){
				System.out.println("Packet recieved from connecting client! Return!");
				return;
			}
	        ...
	        if (category == s_playerPickUp){
	        	if (!player.canCollect) return;
	        	int id = blob.getInt();
	        	Entity e = server.idHandler.get(id);
	        	float effectiveRange = (float) (2 + (Math.sqrt(player.size)/2f * sizeMultiplier) * pickupTolerance);
	        	if (e != null){
	        		if (player.pos.distSquared(e.pos) < effectiveRange * effectiveRange){
	        			...
	        			System.out.println("Out of range! " + player.pos.dist(e.pos) + " > " + effectiveRange);
	        		}
	        	}
	        	return;
	        }
    ...
    public void Send(Object... args){
    	ConvertToBytesAndSave(args);
    	//new sendingThread(args).start();
    }
    
    
    ByteArrayOutputStream buf = new ByteArrayOutputStream();
    
    // You must be VERY VERY CAREFUL to make sure variables going into this function are of the type you want.
    public void ConvertToBytesAndSave(Object... args){
    	if (conn == null || conn.isConnecting() || conn.isClosed() || !conn.isOpen()) return;
    	if (args.length == 1 && args[0] instanceof byte[]) // Don't be smart if we already have what we want
			try {
				synchronized(buf){
