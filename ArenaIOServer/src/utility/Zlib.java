...
public class Zlib {
	...
	static private final int LEVEL = 9;
	...
	public byte[] compress(byte[] buf, int start, int len, byte[] output) {
		deflate.next_in = buf;
		...
		byte[] toRet = new byte[outputlen];
		System.arraycopy(deflate_tmpbuf, 0, toRet, 0, outputlen);
